...
def click():
    text = entry.get()
    output.delete(0.0, END)     # 0.0 : 위치 0행.0열 - 글자
    output.insert(END, text)    # 입력된 문자열의 최종지점까지 삭제 후 출력상자에 데이터를 넣음

root = Tk()   # 루트 생성
root.title("윈도우 창")
root.geometry("250x100+300+300")   # 창의 크기(width x height + x좌표 + y좌표)

frame = Frame(root)   # root를 매개로 frame 객체 생성
frame.pack()    # 화면에 라벨, 버튼 출력(가운데 정렬)

Label(frame, text="이름 : ").grid(row=0, column=0)    # 1행 1열
...
output = Text(frame, width=25, height=3)    # width : 글자의 개수-열의 수, height : 줄의 수
